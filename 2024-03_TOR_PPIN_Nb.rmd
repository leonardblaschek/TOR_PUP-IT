---
title: "TORC PUP-IT in *N. benthamiana*"
...
This script requires MMseqs2 (<https://github.com/soedinglab/MMseqs2>) to be installed on the system. Adjust the `use_python()` call in the setup chunk to ensure you're running in the correct python environment.
...
# Adjust this to point to your python executable with access to MMseqs2
use_python("/home/leonard/Applications/miniconda3/envs/blast/bin/python")
...
```{r}
## Set working folder
data_path <- "data/Nb_peptides/"
out_path <- "data/Nb_output/"
  ...
  for (i in 1:length(data_files)) {
    ...
    protein_int <- as_tibble(assay(pe[["protein"]]), rownames = "protein") |>
      ...
      separate_wider_delim(
        group,
        delim = "_",
        names = c("species", "construct", "bait", "replicate")
      )
...
## Find Arabidopsis homologs for Nb proteins
...
proteome <- as_tibble(
  unlist(
    read.fasta(
      "data/proteome/12864_2019_6058_MOESM10_ESM.fasta",
      seqtype = "AA",
      as.string = TRUE
    )
  ),
...
proteins <- proteome |> 
  filter(protein %in% FC$protein)


# Write fasta file with aa sequences
seqs <- as.list(pull(proteins, sequence))
names <- pull(proteins, protein)
write.fasta(seqs, names, paste0(out_path, "hits.fasta"),
            open = "w", as.string = FALSE
            )

# Mmseqs2 to speed up homology search
system(paste0("mmseqs easy-search ", out_path, "hits.fasta data/proteome/FP_At_ref.fasta ", out_path, "mmseqs2_out.tsv ", out_path, "/temp -s 7.5"))

# Merge data frames
blast <- read_tsv(paste0(out_path, "mmseqs2_out.tsv"),
                  col_names = c("query","target","fident","alnlen","mismatch","gapopen","qstart","qend","tstart","tend","evalue","bits")) |>
         group_by(query) |>
         mutate("mmseqs2_rank" = row_number()) |>
         filter(mmseqs2_rank == 1) |>
         right_join(FC, by = join_by("query" == "protein")) |> 
  dplyr::rename("protein" = "target")

# Export differential enrichment
  write_tsv(
    blast,
    paste0(out_path, input, "_maxLFQ-", maxLFQ, "_hurdle-", hurdle, "_glMinProb-", global_MinProb, "_PPI_FC_homologs.tsv")
  )


```

## Add annotations for At homologs

```{r}
FC <- read_tsv(paste0(out_path, input, "_maxLFQ-", maxLFQ, "_hurdle-", hurdle, "_glMinProb-", global_MinProb, "_PPI_FC_homologs.tsv"))
...
complete <- FC |>
  ...
  mutate(protein = case_when(is.na(protein) ~ query,
                             TRUE ~ protein))

write_tsv(complete, paste0(out_path, input, "_maxLFQ-", maxLFQ, "_hurdle-", hurdle, "_glMinProb-", global_MinProb, "_PPI_FC_annotated.tsv"))

xlsx_export <- complete |>
  arrange(group1, group2, desc(logFC)) |>
  group_by(group1, group2) |>
  group_split()
writexl::write_xlsx(xlsx_export, paste0(out_path, input, "_maxLFQ-", maxLFQ, "_hurdle-", hurdle, "_glMinProb-", global_MinProb, "_PPI_FC_annotated.xlsx"))
```
